...
#include "lpuyat_light.h"
#include "esphome/core/helpers.h"

namespace esphome {
namespace lpuyat {

static const char *const TAG = "lpuyat.light";

void LPUyatLight::setup() {
  if (this->color_temperature_id_.has_value()) {
    this->parent_->register_listener(*this->color_temperature_id_, [this](const LPUyatDatapoint &datapoint) {
  ...
  if (this->dimmer_id_.has_value()) {
    this->parent_->register_listener(*this->dimmer_id_, [this](const LPUyatDatapoint &datapoint) {
      if (this->state_->current_values != this->state_->remote_values) {
        ESP_LOGD(TAG, "Light is transitioning, datapoint change ignored");
        return;
      }
  ...
  if (color_id_.has_value()) {
    this->parent_->register_listener(*this->color_id_, [this](const LPUyatDatapoint &datapoint) {
      if (this->state_->current_values != this->state_->remote_values) {
        ESP_LOGD(TAG, "Light is transitioning, datapoint change ignored");
        return;
      }
      ...
      switch (*this->color_type_) {
        case LPUyatColorType::RGBHSV:
        case LPUyatColorType::RGB: {
...
void LPUyatLight::dump_config() {
  ESP_LOGCONFIG(TAG, "LPUyat Dimmer:");
  if (this->dimmer_id_.has_value()) {
...
light::LightTraits LPUyatLight::get_traits() {
...
void LPUyatLight::setup_state(light::LightState *state) { state_ = state; }

void LPUyatLight::write_state(light::LightState *state) {
