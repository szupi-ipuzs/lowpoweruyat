...
from .. import lpuyat_ns, CONF_LPUYAT_ID, LPUyat, LPUyatDatapointType
...
LPUyatNumber = lpuyat_ns.class_("LPUyatNumber", number.Number, cg.Component)

DATAPOINT_TYPES = {
    "int": LPUyatDatapointType.INTEGER,
    "uint": LPUyatDatapointType.INTEGER,
    "enum": LPUyatDatapointType.ENUM,
}
...
CONFIG_SCHEMA = cv.All(
    number.number_schema(LPUyatNumber)
    .extend(
        {
...
async def to_code(config):
    ...
    parent = await cg.get_variable(config[CONF_LPUYAT_ID])
    cg.add(var.set_lpuyat_parent(parent))
